...
async function write_domains_handler() {
    ...
    try {
        ...
        ui.addNotification(null,
                           E('p', {},
                             _('Unable to write to domains file') + ' ' +
                                 domains_path + ' "' + err.message + '"'));
    }
    ui.hideModal();
    select_handler();
}
...
function read_domains_handler(data) {
    ...
    const section_descr_div =
        E('div', {class : 'cbi-section-descr'},
          _('Domain count in file:') + ' ' + text_data.length);
    ...
    text_data.forEach(function(
        element) { domains_textarea.value += element + '\n' });

    const btn_write_domains = E(
        'button',
        {class : 'cbi-button cbi-button-apply', click : write_domains_handler},
        _('Write domains'));
...
function select_handler() {
    section_data.innerHTML = '';
    const domains_path = section_routes.selectedOptions[0].label;
    fs.read_direct(domains_path)
        .then(read_domains_handler)
        .catch(function(err) {
            if (err.message == 'Failed to stat requested path') {
                fs.exec('/bin/mkdir', [ '/etc/antiblock' ])
                    .then(fs.write(domains_path, '')
                              .then(read_domains_handler(""))
                              .catch(function(err) {
                                  section_data.appendChild(
                                      E('p', {},
                                        _('Unable to create domains file') +
                                            ' ' + domains_path + ' "' +
                                            err.message + '"'));
                              }));
            } else {
                section_data.appendChild(E('p', {},
                                           _('Unable to read domains file') +
                                               ' ' + domains_path + ' "' +
                                               err.message + '"'));
            }
        });
...
return view.extend({
    ...
    handleSave : null,
    handleReset : null,
    load : async function() { return await uci.load('antiblock'); },
    render : function() {
        const uci_routes = uci.sections('antiblock', 'route');

        section_routes =
            E('select', {class : 'cbi-input-select', change : select_handler});
        uci_routes.forEach(function(route) {
            if (route.domains_path.substring(0, 4) != 'http') {
                const routes_option = E('option', {value : route.domains_path},
                                        route.domains_path);
        ...
        if (section_routes.innerHTML != '') {
            ...
            routes_div.appendChild(
                E('div', {class : 'cbi-section-descr'}, _('Domains path:')));
            ...
            routes_div.appendChild(
                E('div', {class : 'cbi-section-descr'},
                  _('Path to file in "Domains path" is not set.')));
            main_div.appendChild(routes_div);
        }
